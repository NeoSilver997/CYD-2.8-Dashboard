...
```bash
cp app/build/esp32.esp32.esp32/app.ino.merged.bin cyd-clock-weather-v2.2.1-4mb.bin
```
...
```
$ python3 -c "d=open('cyd-clock-weather-v2.2.1-4mb.bin','rb').read(); \
...
```bash
shasum -a 256 cyd-clock-weather-v2.2.1-4mb.bin > SHA256SUMS
```
...
```bash
esptool --port /dev/cu.usbserial-1440 --baud 460800 write-flash 0x0 cyd-clock-weather-v2.2.1-4mb.bin
```
